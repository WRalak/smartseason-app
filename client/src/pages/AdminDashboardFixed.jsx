...
import { Users, Activity, BarChart3, TrendingUp, Eye, AlertTriangle, CheckCircle } from 'lucide-react';
...
function AdminDashboard() {
  ...
  const recentActivity = allUpdates.slice(0, 10).map(update => ({
    ...
    stageChange: `${update.previous_stage} to ${update.new_stage}`
  }));
          ...
          {agentMetrics.map((metric) => (
            <div key={metric.agentId} className="flex items-center justify-between p-4 border-b">
              ...
              <div className="text-right text-sm">
                <p className="text-green-600">{Math.round(metric.efficiency * 100)}% efficiency</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {recentActivity.map((activity, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div>
                ...
                <span className="text-sm text-gray-500 ml-2">{activity.timeAgo}</span>
              </div>
